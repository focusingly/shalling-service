...
func newAdminUser(db *gorm.DB, opts ...gen.DOOption) adminUser {
	...
	_adminUser.Hide = field.NewInt(tableName, "hide")
	...
	_adminUser.Link = field.NewString(tableName, "link")
...
func (a *adminUser) updateTableName(table string) *adminUser {
	...
	a.Hide = field.NewInt(table, "hide")
...
type IAdminUserDo interface {
	...
	SelectWithSorts(condList []model.WhereCond, sortList []model.SortColumn) (result []model.AdminUser, err error)
}
...
//		{{end}}
//	{{end}}
func (a adminUserDo) SelectWithSorts(condList []model.WhereCond, sortList []model.SortColumn) (result []model.AdminUser, err error) {
	var params []interface{}

	var generateSQL strings.Builder
	generateSQL.WriteString("SELECT * FROM admin_users ")
	if len(condList) > 0 {
		generateSQL.WriteString("WHERE ")
		for index, cond := range condList {
			if index < len(condList)-1 {
				params = append(params, cond.Val)
				generateSQL.WriteString(a.Quote(cond.Column) + " = ? AND ")
			} else {
				params = append(params, cond.Val)
				generateSQL.WriteString(a.Quote(cond.Column) + " = ? ")
			}
		}
	}
	if len(sortList) > 0 {
		...
		for index, sort := range sortList {
			if index < len(sortList)-1 {
				if sort.Desc {
					generateSQL.WriteString(a.Quote(sort.Column) + " DESC, ")
				} else {
					generateSQL.WriteString(a.Quote(sort.Column) + " ASC, ")
				}
			} else {
				if sort.Desc {
					generateSQL.WriteString(a.Quote(sort.Column) + " DESC ")
