...
type ITagDo interface {
	...
	SelectWithSorts(condList []model.WhereCond, sortList []model.SortColumn) (result []model.Tag, err error)
}
...
//		{{end}}
//	{{end}}
func (t tagDo) SelectWithSorts(condList []model.WhereCond, sortList []model.SortColumn) (result []model.Tag, err error) {
	...
	if len(condList) > 0 {
		...
		for index, cond := range condList {
			if index < len(condList)-1 {
				...
				generateSQL.WriteString(t.Quote(cond.Column) + " = ? AND ")
			} else {
				params = append(params, cond.Val)
				generateSQL.WriteString(t.Quote(cond.Column) + " = ? ")
			}
		}
	}
	if len(sortList) > 0 {
		...
		for index, sort := range sortList {
			if index < len(sortList)-1 {
				if sort.Desc {
					generateSQL.WriteString(t.Quote(sort.Column) + " DESC, ")
				} else {
					generateSQL.WriteString(t.Quote(sort.Column) + " ASC, ")
				}
			} else {
				if sort.Desc {
					generateSQL.WriteString(t.Quote(sort.Column) + " DESC ")
